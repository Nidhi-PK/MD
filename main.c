...
void selectMuxPin (int pin)
{
	...
	{
		if (pin & (1<<z))
			HAL_GPIO_WritePin(selectpinsport, selectPins[z], GPIO_PIN_SET);
		else
			HAL_GPIO_WritePin(selectpinsport, selectPins[z], GPIO_PIN_RESET);
	}
}
...
/*
	  for(int j=0;j<1000;j++)
	  {
		  printf("%dA%dA%dA%dA%dA%dA%dA%dA\n",sensor_value[0][j],sensor_value[1][j],sensor_value[2][j],sensor_value[3][j],sensor_value[4][j],sensor_value[5][j],sensor_value[6][j],sensor_value[7][j]);
	  }
*/
    /* USER CODE END WHILE */
